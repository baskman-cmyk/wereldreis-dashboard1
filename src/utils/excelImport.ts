...
export function applyExcelImport(data: TripDataState, preview: ExcelImportPreview, replace: boolean): TripDataState {
  const merge = <T,>(current: T[], incoming: T[]) => incoming.length ? (replace ? incoming : [...current, ...incoming]) : current;
  ...
  return {
    ...
    categoryBudgets: preview.categoryBudgets.length ? (replace ? preview.categoryBudgets : [...data.categoryBudgets, ...preview.categoryBudgets]) : data.categoryBudgets,
    ...
    overview: firstDay ? {
      ...
      nextFlight: preview.flights[0] ? { ...data.overview.nextFlight, flightNumber: preview.flights[0].flightNumber, airline: preview.flights[0].airline, fromCode: preview.flights[0].fromCode || preview.flights[0].fromCity, toCode: preview.flights[0].toCode || preview.flights[0].toCity, departureTime: preview.flights[0].departureTime, gate: preview.flights[0].gate, seat: preview.flights[0].seat } : data.overview.nextFlight,
    } : data.overview,
  };
}
