...
import { Activity, AlertTriangle, Backpack, CheckCircle2, FileSpreadsheet, Hotel, ListChecks, MapPinned, Plane, ShieldCheck, Upload, WalletCards, X } from "lucide-react";
...
export const ImportCenterModal: React.FC<ImportCenterModalProps> = ({ isOpen, onClose, data, onDataLoaded }) => {
  ...
  const importCount = useMemo(() => preview ? preview.timeline.length + preview.accommodations.length + preview.flights.length + preview.activities.length + preview.checklists.length + preview.documents.length + preview.packingItems.length : 0, [preview]);
