...
import { ExpenseItem, CategoryBudget, BudgetDashboardData } from "../types";
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      {budgetDashboard && (
        <section className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
            {[
              ["Kosten thuis", budgetDashboard.homeCostsEur, "Tijdens 5 maanden reis"],
              ["Kosten vooraf", budgetDashboard.upfrontCostsEur, "Uitrusting, visa, verzekeringen"],
              ["Kosten op reis", budgetDashboard.travelCostsEur, "Verblijf, vervoer, dagbudget"],
              ["Onvoorzien", budgetDashboard.contingencyEur, "Reserve"],
              ["Totaal benodigd", budgetDashboard.totalNeededEur, "Volgens Excel"],
              ["Al betaald", budgetDashboard.alreadyPaidEur, "Reeds voldaan"],
            ].map(([label, value, sub]) => (
              <div key={String(label)} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">{label}</p>
                <p className="mt-1 text-xl font-black text-slate-900 dark:text-white">€{Number(value).toLocaleString("nl-NL", { maximumFractionDigits: 0 })}</p>
          ...
          <div className="grid gap-4 xl:grid-cols-[1.25fr_1fr]">
            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <h3 className="font-black text-slate-900 dark:text-white">Dagbudget per land</h3>
                  <p className="text-xs text-slate-500">Rechtstreeks uit {budgetDashboard.sourceSheet}.</p>
                </div>
                <span className="rounded-xl bg-[#174A7E]/10 px-3 py-1 text-xs font-black text-[#174A7E] dark:text-[#39B8C8]">{budgetDashboard.countryDailyBudgets.length} bestemmingen</span>
              </div>
              <div className="mt-4 overflow-x-auto">
                <table className="w-full min-w-[560px] text-left text-xs">
                  <thead><tr className="border-b border-slate-200 text-[10px] uppercase text-slate-400 dark:border-slate-800"><th className="py-2">Land</th><th>Dagen</th><th>Per dag</th><th>Totaal</th></tr></thead>
                  <tbody>{budgetDashboard.countryDailyBudgets.map((item) => <tr key={item.country} className="border-b border-slate-100 dark:border-slate-800"><td className="py-3 font-bold text-slate-900 dark:text-white">{item.country}</td><td>{item.days}</td><td>€{item.dailyBudgetEur.toLocaleString("nl-NL")}</td><td className="font-black">€{item.totalEur.toLocaleString("nl-NL")}</td></tr>)}</tbody>
                </table>
              </div>
            </div>

            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <h3 className="font-black text-slate-900 dark:text-white">Bekostiging</h3>
              <p className="text-xs text-slate-500">Welke potten de reis financieren.</p>
              <div className="mt-4 space-y-2">{budgetDashboard.fundingLines.map((line) => <div key={line.label} className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2 text-xs dark:bg-slate-800"><span>{line.label}</span><strong>€{line.amountEur.toLocaleString("nl-NL")}</strong></div>)}</div>
              <div className="mt-3 flex items-center justify-between rounded-2xl bg-[#174A7E] px-4 py-3 text-white"><span className="text-xs font-bold">Totaal beschikbaar</span><strong className="text-lg">€{budgetDashboard.fundingTotalEur.toLocaleString("nl-NL")}</strong></div>
            </div>
          </div>
        </section>
      )}
      ...
      <section className="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center gap-2"><PieChart className="h-5 w-5 text-[#39B8C8]" /><h3 className="font-black text-slate-900 dark:text-white">Waar komen de bedragen vandaan?</h3></div>
          <p className="mt-1 text-xs text-slate-500">Begroot komt uit het tabblad Budget. Werkelijk komt uit betaalde regels uit Planning simpel, Vluchten en vervoer en Excursies en activiteiten.</p>
          <div className="mt-4 grid grid-cols-3 gap-2 text-center">
            <div className="rounded-2xl bg-slate-50 p-3 dark:bg-slate-800"><p className="text-lg font-black text-slate-900 dark:text-white">{categoryBudgets.length}</p><p className="text-[10px] font-bold uppercase text-slate-400">budgetregels</p></div>
            <div className="rounded-2xl bg-slate-50 p-3 dark:bg-slate-800"><p className="text-lg font-black text-slate-900 dark:text-white">{expenses.length}</p><p className="text-[10px] font-bold uppercase text-slate-400">uitgavenregels</p></div>
            <div className="rounded-2xl bg-slate-50 p-3 dark:bg-slate-800"><p className="text-lg font-black text-slate-900 dark:text-white">€{remainingEur.toLocaleString("nl-NL", { maximumFractionDigits: 0 })}</p><p className="text-[10px] font-bold uppercase text-slate-400">resterend</p></div>
          </div>
        </div>
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center gap-2"><ListFilter className="h-5 w-5 text-[#174A7E] dark:text-[#39B8C8]" /><h3 className="font-black text-slate-900 dark:text-white">Controleer de bronregels</h3></div>
          <p className="mt-1 text-xs text-slate-500">Klik hieronder op een categorie. De uitgavenlijst wordt direct gefilterd, zodat je precies ziet welke regels meetellen.</p>
          <button onClick={() => { setSelectedCategory("all"); setSearchQuery(""); }} className="mt-4 rounded-xl bg-[#174A7E] px-4 py-2.5 text-xs font-black text-white">Toon alle bronregels</button>
        </div>
      </section>
        ...
        {categoryBudgets.map((cat) => {
          ...
          return (
            <button
              ...
              className={`bg-white dark:bg-slate-900 rounded-3xl p-4 border text-left transition shadow-2xs ${selectedCategory === cat.category ? "border-[#39B8C8] ring-2 ring-[#39B8C8]/20" : "border-slate-200 dark:border-slate-800"}`}
            >
              ...
              <p className="mt-1 text-[10px] font-bold text-slate-400">{expenses.filter((item) => item.category === cat.category).length} bronregels</p>
