...
import { CheckSquare, Check, Search, Plane, ArrowRightLeft, AlertTriangle, Circle, ChevronDown } from "lucide-react";
            ...
            <div key={label} className="overflow-hidden rounded-3xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
              ...
              {isOpen && (
                <div className="space-y-2 border-t border-slate-100 p-3 dark:border-slate-800">
                  {items.map((item) => (
                    <button key={item.id} onClick={() => onToggleCheckItem(item.category, item.id)} className={`flex w-full items-start gap-3 rounded-2xl border p-3 text-left transition ${item.completed ? "border-emerald-200 bg-emerald-50/60 dark:border-emerald-900 dark:bg-emerald-950/20" : "border-slate-200 bg-slate-50 hover:border-[#39B8C8] dark:border-slate-700 dark:bg-slate-800/50"}`}>
