...
export const PaklijstView: React.FC<PaklijstViewProps> = ({
  ...
  const groupedFiltered = useMemo(() => {
    ...
    filtered.forEach((item) => groups.set(item.categorie, [...(groups.get(item.categorie) || []), item]));
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <section className="space-y-3">
        {groupedFiltered.map(([categoryName, categoryItems]) => {
          const CategoryIcon = categoryIcons[categoryName as PackingItem["categorie"]] || PackageCheck;
          const done = categoryItems.filter((item) => isPacked(item.status)).length;
          const isOpen = openCategories.includes(categoryName) || groupedFiltered.length === 1;
          return (
            <div key={categoryName} className="overflow-hidden rounded-3xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
              <button onClick={() => toggleCategory(categoryName)} className="flex w-full items-center gap-3 p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800/60">
                ...
                <div className="min-w-0 flex-1"><p className="font-black text-slate-900 dark:text-white">{categoryName}</p><p className="text-xs font-semibold text-slate-500">{done} van {categoryItems.length} ingepakt</p></div>
                <div className="h-2 w-24 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><div className="h-full bg-[#39B8C8]" style={{ width: `${categoryItems.length ? (done / categoryItems.length) * 100 : 0}%` }} /></div>
              ...
              {isOpen && (
                <div className="grid gap-2 border-t border-slate-100 p-3 dark:border-slate-800 md:grid-cols-2">
                  {categoryItems.map((item) => {
                    ...
                    return (
                      <article key={item.id} className={`flex items-center gap-3 rounded-2xl border p-3 ${isPacked(item.status) ? "border-emerald-200 bg-emerald-50/60 dark:border-emerald-900 dark:bg-emerald-950/20" : "border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/50"}`}>
                        <StatusIcon className="h-4 w-4 shrink-0 text-slate-500" />
                        <div className="min-w-0 flex-1"><p className={`text-sm font-bold text-slate-900 dark:text-white ${isPacked(item.status) ? "line-through opacity-65" : ""}`}>{item.item}</p><p className="text-[10px] font-semibold text-slate-500">{item.toegewezenAan}</p></div>
                        <select value={item.status} onChange={(event) => onUpdateStatus(item.id, event.target.value as PackingItem["status"])} className="max-w-28 rounded-xl border border-slate-200 bg-white px-2 py-2 text-[11px] font-bold dark:border-slate-700 dark:bg-slate-900">
                          {statuses.map((status) => <option key={status.value} value={status.value}>{status.label}</option>)}
                        </select>
                      </article>
                    );
